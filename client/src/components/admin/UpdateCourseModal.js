...
export default function UpdateCourseModal({ onClose, course, onUpdate }) {
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) { newErrors.name = "Course name is required"; valid = false; }
    if (!form.description.trim()) { newErrors.description = "Description is required"; valid = false; }
    if (!form.image.trim()) { newErrors.image = "Course image is required"; valid = false; }
    if (form.price === "" || isNaN(form.price)) { newErrors.price = "Valid price is required"; valid = false; }
      ...
      >
        ...
        >
          ...
          <div style={{ padding: "24px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <h2 style={{ fontSize: "20px", fontWeight: 700, color: "#0f172a", margin: 0 }}>Update Course</h2>
              <p style={{ fontSize: "13px", color: "#64748b", margin: "4px 0 0 0" }}>Modify course information</p>
            </div>
            <button
              onClick={onClose}
              style={{
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                ...
                <div style={{ display: "flex", gap: "16px", alignItems: "center" }}>
                  ...
                  <div style={{ width: "120px", height: "80px", borderRadius: "10px", background: "#f1f5f9", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", border: "1px dashed #cbd5e1", flexShrink: 0 }}>
                    {previewImage ? (
                      <img src={previewImage} alt="Preview" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={(e) => { e.target.style.display = 'none'; }} />
                    ) : (
